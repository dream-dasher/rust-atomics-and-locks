...
mod tests {
       ...
       fn test_partial_reveal() {
              ...
              let hidden = HiddenValue::builder().value(secret).obf_string(TEST_OBF_STRING).build().unwrap();
...
mod insta_tests {
       ...
       fn insta_test_string_hidden_value_with_obfuscation_debug() {
              ...
              let hidden_str = HiddenValue::builder().value(TEST_VALUE_STR).obf_string(TEST_OBF_STR).build().unwrap();
              insta::assert_debug_snapshot!(hidden_str, @r#"HiddenValue { REDACTED.."gaga" }"#);

              const TEST_VALUE_STR_GREEK: &str = "αβγαγα";
              const TEST_OBF_STR_GREEK: &str = "γα";
              let hidden_str_greek = HiddenValue::builder().value(TEST_VALUE_STR_GREEK).obf_string(TEST_OBF_STR_GREEK).build().unwrap();
              insta::assert_debug_snapshot!(hidden_str_greek, @r#"HiddenValue { REDACTED.."γα" }"#);

              const TEST_VALUE_NUM: i32 = -910_050_019;
              const TEST_OBF_NUM: &str = "0_019";
              let hidden_num = HiddenValue::builder().value(TEST_VALUE_NUM).obf_string(TEST_OBF_NUM).build().unwrap();
